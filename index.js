...
router.post('/downloadmp3', async (req, res, next) => {
    try {
        ...
        const wstream = await fs.createWriteStream(`./${fileName}.mp3`, {flags: 'w'});
        ytdl(youtubeLink, { filter: 'audioonly', quality: 'highestaudio', highWaterMark: 1 << 25 })
            .pipe(await wstream.on('data', (data) => {wstream.write(data)}));
...
router.post('/downloadmp3-v2', async (req, res, next) => {
    try {
        ...
        ytdl(youtubeLink, { filter: 'audioonly', quality: 'highestaudio', highWaterMark: 1 << 25 })
            .pipe(await wstream.on('data', (data) => {wstream.write(data)}));
        const rstream = await fs.createReadStream(`./${fileName}.mp3`, {flags:'r'}); // 저장한 파일을 읽은 후 그 값을 보냄
        res.writeHead(206, {
